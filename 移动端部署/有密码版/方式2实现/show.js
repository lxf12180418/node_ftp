...
// const defaultPath = '/storage/emulated/0/要展示的其他目录'; // 设置要展示的其他目录
...
if(!param.user || !param.pwd){
   res.send('error:请输入用户名或密码再请求哦！');
   return;
}

if(param.user != user || param.pwd != pwd){
   res.send('msg:用户名或密码有误，请检查后再次输入！');
   return;
}

res.send('link:'+'./public/fkxqxvw50kksl.js'); // 返回请求的JS链接
...
// 判断目录是否含/storage/emulated/0，含则返回没有权限
...
if (/\.(txt|md|html|json|xml|js?)$/i.test(extname)) {
  ...
  fs.readFile(link, 'utf-8', function(err, data) {
   ...
   // 读取视频等其他文件
  // 给客户端发送需要请求的地址
  res.send('./other?path='+link.replace(defaultPath,''));
}
});	
